...
exports.craw = function() {
            ...
            $(".et_pb_module").each(function(i, items) { 
                $(items).find('tr').each(function(i, item) {                    
                    ...
                    if (mainTitle !== 'FESTA') {                        
                        ...
                        if (dateFormatted !== null && dateFormatted.indexOf('--') === -1) {
                            itemNumber++;
    ...
    function saveFesta(title, address, date, latitude, longitude) {
        var saveData = properties.get('server.save.data') ? properties.get('server.save.data') : false;
        if (saveData) {
    ...
    function convertCustomToDate(customDate) {
        var month = '';
        if (customDate[1]) {
            var literalMonth = customDate[1].toLowerCase().trim();
            switch (literalMonth) {
                case 'gen' | 'ene':
                    month = '01';
                    break;
            ...
            var dateFormatted =  customDate[0].trim() + '-' + month + '-2018';        
